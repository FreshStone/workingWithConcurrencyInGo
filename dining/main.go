...
func dine() {
	...
	completionOrder := make([]string, len(philosophers))
	...
	for i := 0; i < len(philosophers); i++ {
		...
		go diningProblem(philosophers[i], wg, forks, seated, completed, &completionOrder)
	}
...
func diningProblem(philosopher Philosopher, wg *sync.WaitGroup, forks map[int]*sync.Mutex, seated *sync.WaitGroup, completed *sync.Mutex, completionOrder *[]string) {
	defer wg.Done()
	...
	// The philosopher has finished eating, so print out a message.
		...
		*completionOrder = append(*completionOrder, philosopher.name)
	completed.Unlock()
}
